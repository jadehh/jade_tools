    ...
    bool encrypt) const
{
    ...
    try {
        ...
        if (update_func(ctx_, outputData.data(), &processedLength,inputData.data(), static_cast<int>(inputData.size())) != 1) {
    ...
    bool encrypt) const
{
    ...
    try {
        ...
        std::vector<unsigned char> outputData;
        result = encrypt ?
            encryptDataToMemory(inputData, outputData) :
            decryptData(inputData, outputData);
