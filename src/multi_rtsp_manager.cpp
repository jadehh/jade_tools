...
public:
  ...
  explicit Impl(CpuFrameCallback  cpu_call_back) : cpu_callback_(std::move(cpu_call_back)) {
    setOpencvLogger();
  }
#ifdef OPENCV_CUDA_ENABLED
  explicit Impl(GpuFrameCallback  gpu_call_back) : gpu_callback_(std::move(gpu_call_back)) {
    setOpencvLogger();
  }
  explicit Impl(const CpuFrameCallback& cpu_call_back,const GpuFrameCallback& gpu_call_back) : cpu_callback_(cpu_call_back),gpu_callback_(std::move(gpu_call_back)) {
    setOpencvLogger();
  }
#endif
  ...
  static int handleError(const int status, const char *func_name,
    ...
    DLL_LOG_ERROR_FL("Opencv",file_name,line) << err_msg << ",status:" << status << ",Function: " << func_name  ;
    // 返回0表示不中断程序执行
    return 0;
}
...
MultiRtspManager::MultiRtspManager() : impl_(nullptr) {}
...
void MultiRtspManager::stopAll() const { impl_->stopAll(); }
